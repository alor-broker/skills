#!/usr/bin/env python3
"""Bridge between Veles OAuth token and gws CLI.
...
def refresh_token(token_data: dict) -> dict:
    ...
    if missing:
        print(f"ERROR: Google token secret is missing required fields: {', '.join(missing)}", file=sys.stderr)
    ...
    set_json_secret(TOKEN_KEY, _normalize_authorized_user_payload(token_data))
    return token_data


def get_valid_token() -> str:
    """Return a valid access token, refreshing if needed."""
    token_data = get_json_secret(TOKEN_KEY)
    if not token_data:
        print("ERROR: No Google token found in Veles encrypted secrets. Run setup.py --auth-url first.", file=sys.stderr)
        sys.exit(1)
