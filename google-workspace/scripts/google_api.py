#!/usr/bin/env python3
"""Google Workspace API CLI for Veles.
...
from google_secrets import TOKEN_KEY, get_json_secret, set_json_secret
...
def _gws_binary() -> str | None:
    override = os.getenv("VELES_GWS_BIN")
    if override:
        return override
    return shutil.which("gws")


def _gws_env(credentials_file: Path) -> dict[str, str]:
    env = os.environ.copy()
...
def _run_gws(parts: list[str], *, params: dict | None = None, body: dict | None = None):
    ...
    with tempfile.TemporaryDirectory(prefix="veles-google-workspace-") as tmp_dir:
        credentials_file = Path(tmp_dir) / "google_token.json"
        credentials_file.write_text(json.dumps(_load_token_payload()), encoding="utf-8")
        result = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
            env=_gws_env(credentials_file),
        )
...
def get_credentials():
    """Load and refresh credentials from encrypted Veles secrets."""
    _ensure_authenticated()

    from google.oauth2.credentials import Credentials
    from google.auth.transport.requests import Request

    creds = Credentials.from_authorized_user_info(_load_token_payload(), _stored_token_scopes())
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Veles")
