...
Set GitLab settings as environment variables. Store `GITLAB_TOKEN` in the Veles secrets panel as `skills.gitlab-skill.env.GITLAB_TOKEN`; `GITLAB_HOST` is ordinary configuration.
...
For internal GitLab installations where HTTPS is not exposed, use the actual reachable scheme in `GITLAB_HOST`. For ALOR GitLab, use `http://git.alor.ru` rather than `https://git.alor.ru`.
...
If you do not use the Veles secrets panel, add values to your shell profile (~/.bashrc, ~/.zshrc, etc.):
```bash
export GITLAB_HOST="https://gitlab.example.com"
export GITLAB_TOKEN="glpat-your-token-here"
```
...
Create a configuration file in your home directory for ordinary host configuration:

```bash
mkdir -p ~/.veles
cat > ~/.veles/gitlab_config.json << 'EOF'
{
  "host": "https://gitlab.example.com"
}
EOF

chmod 600 ~/.veles/gitlab_config.json  # Restrict file permissions
```

**Location:** `~/.veles/gitlab_config.json`

Keep `GITLAB_TOKEN` in Veles secrets (`skills.gitlab-skill.env.GITLAB_TOKEN`) or an environment variable. The loader can combine `host` from this file with the secret-backed token injected into `GITLAB_TOKEN`. Legacy files that include `access_token` still work, but they are not recommended.
...
1. **Environment Variables** (`GITLAB_HOST`, `GITLAB_TOKEN`) - Highest Priority. `GITLAB_TOKEN` should come from Veles secrets when possible.
2. **User Config File** (`~/.veles/gitlab_config.json`) for `host`, combined with `GITLAB_TOKEN` from Veles secrets or environment variables.
...
**DO:**
✅ Use Veles secrets or environment variables for tokens
✅ Store user config in `~/.veles/gitlab_config.json`
✅ Restrict file permissions: `chmod 600 ~/.veles/gitlab_config.json`
...
2. Check user config: `cat ~/.veles/gitlab_config.json`
...
**Problem:** `Connection refused` or another network error for an `https://` host

**Solutions:**
1. Verify whether the GitLab installation actually exposes HTTPS.
2. If the internal host serves GitLab only over HTTP, set `GITLAB_HOST` to the `http://` URL.
3. For ALOR GitLab, use `GITLAB_HOST=http://git.alor.ru`.
4. The helper does not automatically downgrade HTTPS to HTTP.
...
1. **Immediate Action:** Move credentials to secure location
   ...
   # Or move only the ordinary host to user config
   mkdir -p ~/.veles
   jq '{host: .host}' scripts/config.json > ~/.veles/gitlab_config.json
   chmod 600 ~/.veles/gitlab_config.json
   ```
